...
impl<T: Clone> Tilemap<T> {
    ...
    pub fn new(map_size: impl Into<Vector>, tile_size: impl Into<Vector>) -> Tilemap<T> {
        let (map_size, tile_size) = (map_size.into(), tile_size.into());
        let data =
            vec![Tile::empty(None); (map_size.x / tile_size.x * map_size.y / tile_size.y) as usize];
        Tilemap {
            data,
            map_size,
            tile_size,
    ...
    pub fn region(&self) -> Rectangle {
        Rectangle::new_sized(self.map_size)
    }
    ...
    pub fn valid(&self, index: impl Into<Vector>) -> bool {
    ...
    pub fn get(&self, index: impl Into<Vector>) -> Option<&Tile<T>> {
    ...
    pub fn get_mut(&mut self, index: impl Into<Vector>) -> Option<&mut Tile<T>> {
        let index = index.into();
        if self.valid(index) {
    ...
    pub fn set(&mut self, index: impl Into<Vector>, value: Tile<T>) {
        let index = index.into();
    ...
    pub fn point_empty(&self, index: impl Into<Vector>) -> bool {
    ...
    pub fn shape_empty(&self, shape: &impl Shape) -> bool {
        ...
        for x in x_start..x_end {
            for y in y_start..y_end {
                ...
                if !self.point_empty(point)
                    && shape.overlaps(&Rectangle::new(point, self.tile_size()))
                {
                    return false;
                }
            }
    ...
    pub fn move_until_contact<U: Shape>(&self, bounds: U, speed: impl Into<Vector>) -> (U, Vector) {
        ...
        let slide_x = |diff: f32, mut attempt: Vector| {
            while diff.abs() > 0.0
                && (attempt.x + diff).abs() <= speed.x.abs()
                && self.shape_empty(&rectangle.translate(attempt + Vector::X * diff))
            {
                attempt.x += diff;
            }
            attempt
        ...
        let slide_y = |diff: f32, mut attempt: Vector| {
            while diff.abs() > 0.0
                && (attempt.y + diff).abs() <= speed.y.abs()
                && self.shape_empty(&rectangle.translate(attempt + Vector::Y * diff))
            {
                attempt.y += diff;
            }
            attempt
